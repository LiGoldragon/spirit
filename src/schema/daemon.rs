...
pub trait ComponentDaemon: Sized + 'static {
    ...
    type ConfigurationError: std::fmt::Display;
    type Engine;
    ...
    /// `Serialize` bounds are what the emitted publisher needs to encode
    /// the subscription-event frame; they mirror `signal_frame`'s own
    /// `StreamingFrame::encode` bounds so the event rides the wire.
...
pub trait DaemonBinder: ComponentDaemon {
    fn bind(
        ...
        if let Some(meta_socket_path) = configuration.meta_socket_path() {
            let socket_mode = configuration.meta_socket_mode().unwrap_or_else(|| SocketMode::new(0o600));
            sockets.push(
...
impl<Daemon: ComponentDaemon> EmittedSubscriptions<Daemon> {
    ...
    fn publish(&self, event: Daemon::StreamEvent) -> Result<usize, FrameError> {
        ...
        // Reborrow the guard once into the owned state so the disjoint
        // field borrows below (`registry` shared, `publisher` exclusive)
        // split cleanly; going through the `MutexGuard` `Deref` directly
        // would conflict because deref yields the whole struct.
        let state = &mut *guard;
        ...
        for (token, frame) in frames {
            match Self::deliver(&mut state.writers, token, frame) {
                Ok(true) => delivered += 1,
                Ok(false) => {}
    ...
    fn deliver(
        writers: &mut std::collections::HashMap<SubscriptionTokenInner, UnixStream>,
        token: SubscriptionTokenInner,
        frame: signal_frame::StreamingFrame<Input, Output, Daemon::StreamEvent>,
    ) -> Result<bool, FrameError> {
        use std::io::Write;
        let Some(writer) = writers.get_mut(&token) else {
            return Ok(false);
        };
        let bytes = frame.encode().map_err(|_| FrameError::Io(std::io::Error::other("subscription frame encode")))?;
