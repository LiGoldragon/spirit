...
impl<'configuration> GuardianPromptBuilder<'configuration> {
    ...
    pub(crate) fn guardian_prompt(
        ...
        let retry_text = retry
            ...
            .unwrap_or_default();
        let few_shot_block = self.guardian_few_shot_block();
        Prompt {
    ...
    fn guardian_few_shot_block(&self) -> &'static str {
        "Examples:\n- Clear single durable testing intent: accept with (Accept).\n- Clear single provider intent: accept with (Accept).\n- Existing live arrow repeats the candidate: reject with reason Duplicate, shaped like (Reject (Duplicate [same forward arrow already exists])).\n- Candidate opposes an existing live arrow: reject with reason Contradiction, shaped like (Reject (Contradiction [conflicts with the bracket-form NOTA rule])).\n- One Entry bundles separable actions: reject with reason Compound, shaped like (Reject (Compound [agent key resolution and Spirit deployment are separate arrows])).\n- Candidate is only uncertainty or status: reject with reason NonIntent, shaped like (Reject (NonIntent [uncertainty about readiness is not durable intent]))."
    }
}
