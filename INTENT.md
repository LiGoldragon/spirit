...
`Query` predicate and durable read behavior. Privacy here is NOMINAL / name-only
(`Zero` public, any nonzero private) in a single database with no real security
fail-safe — it narrows the intended audience, it does not protect the content.
Treat all Spirit data as potentially exposed and keep genuinely sensitive
material in actually-private storage (private repositories), not Spirit; the
feature may later become secure-private (see ARCHITECTURE.md, Known limits).
