...
fn transport_rejects_length_prefixed_raw_nota_text() {
    let nota =
        b"(Record ([[socket-negative]] Decision [text must not be daemon wire] Maximum Minimum 1 Zero))";
...
fn generated_input_decoder_rejects_raw_nota_text_directly() {
    let nota =
        b"(Record ([[socket-negative]] Decision [text must not be signal frame] Maximum Minimum 1 Zero))";
