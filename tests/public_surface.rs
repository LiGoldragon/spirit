...
fn crate_root_does_not_reexport_generated_plane_nouns() {
    ...
    // The three generated *plane* modules carry the wire/runtime nouns and must
    // stay reached through `spirit::schema::{signal,nexus,sema}`. The emitted
    // daemon entry-point module (`schema::daemon`) is the daemon's public API —
    // `DaemonCommand` / `DaemonEntry` / `DaemonError` / `ComponentDaemon` — and
    // is re-exported at the crate root deliberately, so it is exempt.
