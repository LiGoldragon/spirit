...
use spirit::schema::signal::{
    ...
    Justification, Kind, Magnitude, Output, Privacy, PrivacySelection, Query, QuoteText, Reasoning,
    RecordRequest, RemovalCandidateCollection, SelectedKind, Testimony, VerbatimQuote,
};
...
fn collect_removal_candidates_archives_to_separate_db_and_removes_from_live() {
    ...
    // Two live records: one matches the owner-selected removal query
    // (`Governing`), one stays (`Meaning`).
    record(&mut engine, entry("governing", "obsolete intent to retire"));
    record(
        &mut engine,
...
fn collect_removal_candidates_archives_and_removes_over_the_meta_socket() {
    ...
    // Two live records over the WORKING socket: one matching the owner-selected
    // removal query (`governing`), one that must remain (`meaning`).
    record_over_socket(
        &working_socket,
        entry("governing", "obsolete intent to retire"),
    );
    record_over_socket(
        &working_socket,
        entry("meaning", "intent that must remain live"),
